...
static void task_info_add_to_list(int pid)
{
	...
	read_lock(&head_rwlock);
	ti = task_info_find_pid(pid);
	if (ti != NULL) {
		ti->timestamp = jiffies;
		atomic_inc(&ti->count);
		read_unlock(&head_rwlock);
		return;
	}
	read_unlock(&head_rwlock);

	ti = task_info_alloc(pid);
	write_lock(&head_rwlock);
	list_add(&ti->list, &head);
	write_unlock(&head_rwlock);
}
...
void task_info_remove_expired(void)
{
	...
	list_for_each_safe (p, q, &head) {
		ti = list_entry(p, struct task_info, list);
		if (jiffies - ti->timestamp > 3 * HZ &&
		    atomic_read(&ti->count) < 5) {
			list_del(p);
			kfree(ti);
		}
	}
